...
# 2 - Model Selection Strategy Enum
...
# 3 - Model Configuration Dataclass
@dataclass
class ModelConfig:
...
# 5 - Model Result Dataclass
@dataclass
class ModelResult:
    """Result container for a fitted model."""
...
# 7 - GLM Model Selector Class
class GLMModelSelector:
    """GLM Model Selector with multiple selection strategies."""
    ...
    def __init__(self, config: ModelConfig):
        ...
        self.all_models: List[ModelResult] = []  # Initialize all_models list
    ...
    def _random_search(self) -> ModelResult:
        ...
        # Verify at least one model converged (moved outside loop)
    ...
    def fit(self) -> ModelResult:
        """Fit the model using the configured selection strategy."""
        if self.train_data is None or self.test_data is None:
            raise ValueError("Data must be prepared before fitting. Call prepare_data() first.")
    ...
    def load_model(cls, filepath: str) -> 'GLMModelSelector':
        ...
        selector = cls(data['config'])
        selector.best_model = ModelResult(
            formula=f"{data['config'].target_column} ~ {' + '.join(data['predictors'])}",
            predictors=data['predictors'],
            model=data['model'],
            metrics=ModelMetrics(**{k: v for k, v in data['metrics'].items()
    ...
    def get_model_summary(self) -> Dict[str, Any]:
        ...
        if self.best_model is None:
            raise ValueError("No model available. Call fit() first.")

        return {
            'formula': self.best_model.formula,
            'predictors': self.best_model.predictors,
            'n_predictors': len(self.best_model.predictors),
            'metrics': self.best_model.metrics.to_dict(),
...
if __name__ == "__main__":
    # Load data
    credit_data = pd.read_csv('credit2.csv')

    # Define predictors
    predictors = [
        'accounts', 'history_credit', 'object_credit', 'savings',
        'employment_old', 'family_status', 'guarantees', 'property',
        'other_credits', 'home_status', 'job_type', 'telephone',
        'duration_credit', 'amount_credit', 'age'
    ]
    ...
    # Prepare data
    selector.prepare_data(credit_data)
    ...
    # Print summary
    print("\nModel Summary:")
    print(f"Best AIC: {best_model.metrics.aic:.2f}")
    print(f"Best AUC: {best_model.metrics.auc:.4f}")
    print(f"Predictors: {best_model.predictors}")
